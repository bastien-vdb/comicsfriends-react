...
function Mesh2(props) {
  const epiccomic = useLoader(GLTFLoader, 'epiccomic/scene.gltf');

  const bdvolante = useRef()
  const data = useScroll()
  useFrame((state, delta) => {
    const scroll1 = data.range(0, 1);
    bdvolante.current.rotation.y = (Math.PI*2) * scroll1;
  })
  
  
  return (
    <Suspense fallback={null}>
      <primitive ref={bdvolante} {...props} object={epiccomic.scene} scale={0.025} />
    </Suspense>
  )
}
